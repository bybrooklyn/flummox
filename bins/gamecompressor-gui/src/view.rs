...
use crate::app::{GameRow, Message, PAGES, Page, State};
use crate::theme;
...
fn sidebar(state: &State) -> Element<'_, Message> {
    let mut items = column![text("gamecompressor").size(18)].spacing(4).padding(16);
    for page in PAGES {
...
fn drives(state: &State) -> Element<'_, Message> {
    ...
    for mountpoint in state.drives() {
        let games_here = state
            ...
            .count();
        let free = gc_core::backend::free_bytes(&mountpoint).unwrap_or(0);
        let entry = column![
